    ...
    return(
        <>
        {
            tech?.map((element,index) => {
                return lineTech?.includes(element.dim) ? (<Image className='project-info-tech-img' key={index} src={element.url} width={32} height={32} alt={element.alt} title={element.alt}></Image>) : null
            })
        }
        </>
    )
