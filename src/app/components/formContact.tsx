...
interface FormContact {
  formData: IFormContact;
}

export default function formContact({ formData }: FormContact) {
  ...
  return (
    <>
      {isEmailSent != null && (
        ...
        >
          {isEmailSent ? formData.success : formData.fail}
        </div>
      )}
      <form
        ref={form}
      ...
      >
        <div className="flex flex-col mb-2">
          <label className="hidden">{formData.names}</label>
          <input
            type="text"
            name="user_name"
            required
            ...
            placeholder={formData.names}
          />
        </div>
        <div className="flex flex-col mb-2">
          <label className="hidden">{formData.email}</label>
          <input
            className="h-12 placeholder:text-sm bg-light-gray rounded-lg border-b-2 border-light-blue-400 px-2.5 outline-light-blue-300 dark:bg-dark-bg dark:border-dark-blue-500 dark:focus:outline-dark-blue-200 dark:focus:bg-dark-items"
            type="email"
            name="user_email"
        ...
        <div className="flex flex-col">
          <label className="hidden">{formData.message}</label>
          <textarea
